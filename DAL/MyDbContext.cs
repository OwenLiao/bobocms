...
namespace DAL
{
    public class MyDbContext : DbContext
    {
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            //文章详情标签多对多关系
            modelBuilder.Entity<DetailArticleCategory>().HasKey(q => new { q.ArticleId, q.CategoryId });
            modelBuilder.Entity<DetailArticleCategory>().HasOne(q => q.Article).WithMany(q => q.DetailCategories).HasForeignKey(q => q.ArticleId);
            modelBuilder.Entity<DetailArticleCategory>().HasOne(q => q.Category).WithMany(q => q.DetaileArticles).HasForeignKey(q => q.CategoryId);
